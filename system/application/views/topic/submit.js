function validate_topic_form()  
{
	...
	//validate tags
	form_rules.push(new Array('#problem_tags', 'The tags field', 'format_list[ ][,]|word_count[3][,]'));
	...
	if(!validate_form(form_rules)) {
		...
		//replace delimiter with white space, server is expecting this.
		tags = $('#problem_tags').val();
		tags = tags.replace(/,(?: )*/g,' ');
		post_data += "&problem_tags=" + tags;
		sj = new Simple_json();
