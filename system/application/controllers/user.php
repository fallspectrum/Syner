...
class User extends Controller {
	...
	*/
	...
	/**
	* This controller function retrieves the users saved tags.
	*/
	function retrieve_tags_json()
	{
		$this->load->model("Users",'',TRUE);
		$this->load->library("Simple_Json");
		$json = new Simple_Json();
		//not logged in
		if($this->user_session->get_privilege() == 0) {
			$json->add_error_response("priv_error",$json->error_codes['privilege_error']);
		}
		
		//we are logged in so lets get our tags.
		else {
			
			$user_id = $this->user_session->get_user_id();

			//returns a array of saved taged descriptors (see users model)
			$stds = $this->Users->retrieve_tags($user_id);
			...
			foreach ($stds as $std) {
				...
				$tmp['id'] = $std->id;
				$tmp['name'] = $std->name;
				$tmp['operator'] = $std->operator;
				array_push($data,$tmp);
			}
			
			$json->add_data("saved_tag_descriptors",$data);
		}
	...
	/**
	* This controller function handles a request to save tags
	* @todo boost tag name lookup performance
	*/
	function save_tags_json() 
	{
		$this->load->library("simple_json");
		$json = new Simple_Json();
